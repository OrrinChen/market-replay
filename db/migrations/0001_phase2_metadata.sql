...
CREATE INDEX idx_event_files_dataset_id ON event_files(dataset_id);
CREATE INDEX idx_replay_jobs_dataset_id ON replay_jobs(dataset_id);
CREATE INDEX idx_replay_jobs_id ON replay_jobs(id);
CREATE INDEX idx_replay_jobs_idempotency_key ON replay_jobs(idempotency_key) WHERE idempotency_key IS NOT NULL;
CREATE INDEX idx_replay_metrics_job_id ON replay_metrics(job_id);
CREATE INDEX idx_validation_errors_job_id ON validation_errors(job_id);

-- +goose Down
DROP TABLE IF EXISTS validation_errors;
DROP TABLE IF EXISTS replay_metrics;
