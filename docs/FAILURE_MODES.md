...
| Missing or stale replay manifest | Quality report has empty hash/row/checkpoint fields or does not match the event file stats. | Treat the report as incomplete and do not use it as benchmark evidence. | Re-run the replay job after confirming worker access to the input file and Postgres write health. |
| Validation-error summary spike | `/validation-errors/summary` shows a sudden count increase by type/file/symbol/day. | Keep replay available, but mark the dataset quality as degraded for that input. | Inspect the lineage page, source file hash, and row-level CSV export before comparing benchmark results. |
